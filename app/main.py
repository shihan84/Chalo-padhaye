...
def _student(token: str, student_id: str):
    ...
    if not r.ok: raise HTTPException(401, "Could not verify student")
    rows = r.json()
    if not rows: raise HTTPException(403, "Student is not available to this parent")
    return rows[0]
...
def _insert(token: str, table: str, payload: dict):
    ...
    if not r.ok: raise HTTPException(502, f"Could not save {table}")
    rows = r.json()
    return rows[0] if rows else None

def _history(token: str, session_id: str, limit: int = 7):
    r = requests.get(f"{SUPABASE_URL}/rest/v1/tutor_messages", headers=_headers(token), params={"session_id": f"eq.{session_id}", "select": "role,message,created_at", "order": "created_at.desc", "limit": str(limit)}, timeout=10)
    if not r.ok: return []
...
def tts(data: TTSIn, authorization: Optional[str] = Header(default=None)):
    ...
    if not FISH_AUDIO_API_KEY or not FISH_AUDIO_VOICE_ID: raise HTTPException(503, "Custom tutor voice is not configured")
    text = data.text.replace("**", "").strip()
    if not text: raise HTTPException(400, "Text is required")
    if len(text) > 1200: text = text[:1200]
    r = requests.post("https://api.fish.audio/v1/tts", headers={"Authorization": f"Bearer {FISH_AUDIO_API_KEY}", "Content-Type": "application/json", "model": FISH_AUDIO_MODEL}, json={"text": text, "reference_id": FISH_AUDIO_VOICE_ID, "format": "mp3"}, timeout=60)
    if not r.ok: raise HTTPException(502, f"Tutor voice generation failed ({r.status_code})")
...
def chat(data: ChatIn, authorization: Optional[str] = Header(default=None)):
    ...
    if data.curriculum not in ("maharashtra", "nios"): raise HTTPException(400, "Unknown curriculum")
    session_id = data.session_id
    if not session_id:
        session = _insert(token, "tutor_sessions", {"student_id": student["id"], "subject": f"{data.curriculum}:{data.subject}"})
        ...
        if not r.ok or not r.json(): raise HTTPException(403, "Session is not available")

    history = _history(token, session_id)
    _insert(token, "tutor_messages", {"session_id": session_id, "role": "student", "message": data.message})
    answer = tutor.reply(data.message, student_name=student["full_name"], grade=int(student["grade"]), subject=data.subject, curriculum=data.curriculum, history=history)
    if answer.get("text"): _insert(token, "tutor_messages", {"session_id": session_id, "role": "tutor", "message": answer["text"]})
